...
void Prop::initializeActionManager()
{
	...
	actions().inputs.addInput("setHealth", [&](Action action) {
		TextItem item = action.parameters;
		setHealth(item.toInt(0));
	});

	actions().inputs.addInput("setMaxHealth", [&](Action action) {
		TextItem item = action.parameters;
		setMaxHealth(item.toInt(0));
	});

	/*actions().inputs.addInput("setCollisionBox", [&](Action action) {
		TextItem item = action.parameters;
		coll
	});*/
}